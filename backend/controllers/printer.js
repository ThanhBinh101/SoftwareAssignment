...
const deletePrinter = async (req, res) => {
  ...
  try {
    ...
    const printerIndex = db.Printer.findIndex(
    ...
    const officerPrinterOwner = db.Officer.find((officer) =>
      officer.printers.includes(printerId)
    );

    officerPrinterOwner.printers = officerPrinterOwner.printers.filter(
    ...
    res
      .status(200)
      .json({ message: `Printer ${printerId} deleted successfully` });
    return;
  } catch (parseError) {
...
const addPrinter = async (req, res) => {
  ...
  if (!id || !location || !officerID) {
    return res
      .status(400)
      .json({ message: `Missing required fields: id, location, officerID` });
  }
  ...
  try {
    ...
    if (officer) {
      ...
      return res
        .status(201)
        .json({ message: "Printer added successfully", newPrinter });
    } else {
    ...
    return res
      .status(500)
      .json({ message: "Error processing printer.", error: err.message });
  }
};
